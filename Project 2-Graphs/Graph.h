...
class Graph {
    private:
         /**
         * The adjacency matrix of the graph represented through an unordered map. 
         * Each key value is the name of a node.
         * Each mapped value is a vector containing the edges connected to that node.
         * Each edge is represented by a pair containg the destination node and the weight of the edge.
         */
        ...
        /**
         * Contains every edge in the graph sorted by increased edge weights.
         */
        vector<tuple<double, string, string>> edges;

    public: 
        /**
         * Use Breadth First Search to populate the nodeDistances hash to help the shortest_path_unweighted function.
         * @param startLabel The source node.
         * @param nodeDistances Contains a pair of (parentNode, nodeDistance).
         */
        void unweightedBFS(string const & startLabel, unordered_map<string, pair<string, int>> & nodeDistances);

        /**
         * Use Breadth First Search to find all nodes connected to the startLabel with a weight >= threshold.
         * @param currLabel The source node.
         * @param threshold The threshold of edge weight for an edge to be considered connected.
         */
        vector<string> componentBFS(string const & startLabel, double const & threshold);

        /**
         * Create an undirected edge within the adjaceny list from node 'u' to node 'v' with weight 'w'.
